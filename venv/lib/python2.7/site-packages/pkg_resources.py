...
class MarkerEvaluation(object):
    ...
    def comparison(cls, nodelist):
        ...
        comp = nodelist[2][1]
        cop = comp[1]
        if comp[0] == token.NAME and len(nodelist[2]) == 3:
            cop = 'not in' if cop == 'not' else 'is not'
        try:
            cop = cls.get_op(cop)
        except KeyError:
            msg = f'{repr(cop)} operator not allowed in environment markers'
            raise SyntaxError(msg)
            ...
            for entry in os.listdir(path_item):
                ...
                if lower.endswith('.egg-info') or lower.endswith('.dist-info'):
                    ...
                    for line in entry_lines:
                        ...
                        yield from find_distributions(path)
                        break
...
class Distribution(object):
                    ...
                    if extra:
                        if ':' in extra:
                            ...
                            if (
                                ...
                                or not invalid_marker(marker)
                                and not evaluate_marker(marker)
                            ):
                                # XXX warn
                                reqs=[]
    ...
    def __repr__(self):
        return "%s (%s)" % (self, self.location) if self.location else str(self)
...
class Requirement:
    ...
    @staticmethod
    def parse(s):
        if reqs := list(parse_requirements(s)):
            if len(reqs)==1:
                return reqs[0]
