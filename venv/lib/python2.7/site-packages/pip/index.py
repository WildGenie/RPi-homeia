...
class PackageFinder(object):
    ...
    def __init__(self, find_links, index_urls,
        ...
        self.allow_external = {normalize_name(n) for n in allow_external}

        # Which names are allowed to install insecure and unverifiable files?
        self.allow_unverified = {normalize_name(n) for n in allow_unverified}
    ...
    def _get_pages(self, locations, req):
        ...
        while all_locations:
            ...
            for link in page.rel_links():
                ...
                if (
                    normalized not in self.allow_external
    ...
    def _package_versions(self, links, search_name):
        for link in self._sort_links(links):
            yield from self._link_package_versions(link, search_name)
    ...
    def _link_package_versions(self, link, search_name):
        ...
        if link.egg_fragment:
            ...
            if ext == wheel_ext:
                ...
                ) and not wheel.supported(tags=supported_tags_noarch):
                    logger.debug(
        ...
        if (
            ...
            and normalize_name(search_name).lower() not in self.allow_external
            and not self.allow_all_external
        ):
        ...
        if (
            ...
            and normalize_name(search_name).lower() not in self.allow_unverified
        ):
...
class HTMLPage(object):
    ...
    def get_page(cls, link, req, cache=None, skip_archives=True, session=None):
        ...
        try:
            if skip_archives:
                if cache is not None and cache.is_archive(url):
                    return None
    ...
    def api_version(self):
        if not hasattr(self, "_api_version"):
            ...
            if metas := [
                x
                for x in self.parsed.findall(".//meta")
    ...
    def rel_links(self):
        yield from self.explicit_rel_links()
        yield from self.scraped_rel_links()
...
def package_to_requirement(package_name):
    ...
    if match := re.search(r'^(.*?)-(dev|\d.*)', package_name):
        name = match.group(1)
