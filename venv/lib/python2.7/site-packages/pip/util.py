...
def rmtree_errorhandler(func, path, exc_info):
    ...
    if (
        (exctype is not WindowsError or value.args[0] != 5)
        and (exctype is not OSError or value.args[0] != 13)
        and (exctype is not PermissionError or value.args[3] != 5)
    ):
        raise
...
def get_installed_distributions(local_only=True,
    ...
    """
    local_test = dist_is_local if local_only else (lambda d: True)
...
def cache_download(target_file, temp_location, content_type):
    ...
    with open(f'{target_file}.content-type', 'w') as fp:
...
def is_prerelease(vers):
    ...
    return any(
        any(y in set(["a", "b", "c", "rc", "dev"]) for y in x) for x in parsed
    )
