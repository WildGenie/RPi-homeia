...
def get_file_content(url, comes_from=None, session=None):
    ...
    if match := _scheme_re.search(url):
        ...
        if scheme == 'file':
            ...
            return (resp.url, resp.text) if six.PY3 else (resp.url, resp.content)
    try:
        f = open(url)
        content = f.read()
    except IOError:
...
def url_to_path(url):
    ...
    path = urllib.unquote(path)
    path = f'{path[0]}:{path[2:]}' if _url_drive_re.match(path) else f'/{path}'
    return path
...
def unpack_http_url(link, location, download_cache, download_dir=None,
    ...
    if download_cache:
        ...
        cache_content_type_file = f'{cache_file}.content-type'
        already_cached = (
...
def unpack_file_url(link, location, download_dir=None):
    ...
    from_path = download_path if already_downloaded else link_path
