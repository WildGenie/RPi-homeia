...
#
BARE_CONSTRAINTS = (
    ((f'({RELOP})?\\s*({VERSPEC})({COMMA}(' + RELOP) + r')\s*(') + VERSPEC
) + '))*'
...
#
CONSTRAINTS = (
    (f'\\(\\s*(?P<c1>{BARE_CONSTRAINTS}|{DIRECT_REF}' + r')\s*\)|(?P<c2>')
    + BARE_CONSTRAINTS
) + '\s*)'


EXTRA_LIST = f'{EXTRA_IDENT}({COMMA}{EXTRA_IDENT})*'
EXTRAS = f'\\[\\s*(?P<ex>{EXTRA_LIST})?\\s*\\]'
REQUIREMENT = (
    f'(?P<dn>{IDENT})\\s*({EXTRAS}\\s*)?(\\s*' + CONSTRAINTS
) + ')?$'
...
#
RELOP_IDENT = f'(?P<op>{RELOP})\\s*(?P<vn>{VERSPEC})'
